...
class Mp4FlacRemuxerTest {
    ...
    private val streamInfo = byteArrayOf(
    ...
    private fun dfLaBox(): ByteArray {
        val payload = ByteArrayOutputStream().apply {
            ...
            write(byteArrayOf(0x80.toByte(), 0x00, 0x00, 0x22))
            write(streamInfo)
        }.toByteArray()
    ...
    private fun buildContainer(withFlac: Boolean, frames: ByteArray): ByteArray {
        ...
        out.write(box("mdat", frameBytes(0x11)))
    ...
    fun unwrapsFlacFramesIntoNativeFlac() {
        ...
        assertThat(bytes.copyOfRange(4, 8)).isEqualTo(byteArrayOf(0x80.toByte(), 0x00, 0x00, 0x22))
        ...
        val expectedAudio = framesA + frameBytes(0x11)
    ...
    fun reportsNonFlacContainerAsNotFlac() {
        ...
        input.writeBytes(buildContainer(withFlac = false, frames = frameBytes(0x0A)))
    ...
    fun detectsFlacContainer() {
        ...
        input.writeBytes(buildContainer(withFlac = true, frames = frameBytes(0x0A)))
    ...
    fun refusesTruncatedContainer() {
        ...
        val full = buildContainer(withFlac = true, frames = frameBytes(0x0A))
