/**
 * SecretsBridge.cpp — Native secret vault for LastWave (TEMPLATE - NO SECRETS).
 *
...
Java_com_lastwave_app_data_lossless_NativeSecrets_nativeBaseUrl(
    JNIEnv* env,
    jclass,
    jobject context) {
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_lastwave_app_data_lossless_NativeSecrets_nativeApiKey(
    JNIEnv* env,
    jclass,
    jobject context) {
    if (!verifyCallerSignature(env, context)) {
...
// Java: com.lastwave.app.data.plugin.NativeModuleKey.nativeModuleKeyBytes
extern "C" JNIEXPORT jbyteArray JNICALL
Java_com_lastwave_app_data_plugin_NativeModuleKey_nativeModuleKeyBytes(
    JNIEnv* env,
    jclass,
    jobject context) {
    if (!verifyCallerSignature(env, context)) {
...
// Java: com.lastwave.app.data.plugin.NativeModuleKey.nativeModuleKeyB64
extern "C" JNIEXPORT jstring JNICALL
Java_com_lastwave_app_data_plugin_NativeModuleKey_nativeModuleKeyB64(
    JNIEnv* env,
    jclass,
    jobject context) {
    if (!verifyCallerSignature(env, context)) {
